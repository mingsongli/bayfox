...
def alphaT(T,S,pH):
    ...
    #
    # Python version by Mingsong Li, June 23, 2021
    #
    # Disclaimer/Notes:
    #
    ...
    # 06/23/21 python version by Mingsong Li
    #============================================#
    #
    ...
    # K1
    A1   =   13.41910 * np.sqrt(S) + 0.0331 * S - 5.33e-5 * S * S
    B1   = -530.12300 * np.sqrt(S) - 6.1030 * S
    C1   =   -2.06950 * np.sqrt(S)

    pK10 = -126.34048 + 6320.813/T + 19.568224 * np.log(T)
    pK1  = pK10 + A1 + B1/T + C1 * np.log(T)
    K1   = np.power(10, -1*pK1)

    # K2
    A2   =   21.0894 * np.sqrt(S) +  0.1248 * S - 3.687e-4 * S * S
    B2   = -772.4830 * np.sqrt(S) - 20.0510 * S
    C2   =   -3.3336 * np.sqrt(S)

    pK20 = -90.18333 + 5143.692/T + 14.613358*np.log(T)
    pK2  = pK20 + A2 + B2/T + C2 * np.log(T)
    K2   = np.power(10, -1*pK2)

    K1p  = np.power(10, -2.98)
    ...
    ad       = np.exp(lnad1000/1000)
    ab       = np.exp(lnab1000/1000)
    ac       = np.exp(lnac1000/1000)


    RH2O = 2e-3 # 2e-3 value irrelevant over
    ...
    # R_Total
    RT =   (rd*zd+ra*za+rb*zb+rc*zc)/(1-(rd*zd+ra*za+rb*zb+rc*zc))
